...
参考文章的纵向模型给出 `I_p*theta_ddot=...-T+Tp` 与 `I_M*phi_ddot=...+Tp`。必须同时核对摆杆向量定义：文章使用“轮轴到机体”，代码使用“机体到轮轴”，故 `theta_article=-theta_code`。结合 MuJoCo 单通道脉冲，物理轮力矩正方向对应文章 `T>0`，物理腿角向力矩负方向对应文章 `Tp>0`。因此主路径必须使用 `lqr_wheel_sign=+1`、`lqr_pitch_sign=-1`，并让有限差分线性化使用同一映射。不可仅按文章方程忽略角度向量方向，也不可只按一次画面翻转符号。
...
### 2026-07-10：短时站立现象不是单一参数结果，也不是平衡证明

- 现象：此前可视化中会出现腿长塌短、低频摆动、前后加速、慢漂，或者倒下后腿长才回到目标；后来短时 `--lqr-true-equilibrium` 数据显示腿长接近 `0.35 m`、分支不凹陷、执行器不饱和。但该短时现象不能证明机器人已经长时间平衡。
...
- 验证：短时 finite smoke 只能说明程序和局部控制链没有立刻崩溃；是否真正平衡必须由长时 MuJoCo 可视化观察确认。
- 下次避免：不要把“短时能立起来”归因于某个单一增益，也不要写成平衡成功。任何后续几何、腿长截面、状态定义或 LQR 权重变化，都必须重新走：解析 IK/Jacobian 门禁 → true equilibrium 门禁 → 同步 `X0/U0/F_l0/J0/contact_force0` → 围绕 `U0` 线性化 → 长时可视化观察。
...
## 2026-07-11：Tp 默认执行方向与当前 LQR/VMC 符号链相反

现象：当前主路径中，车体向后倒时虚拟腿反而向前摆，`Tp` 通道看起来在帮助失稳模态而不是纠正它。

根因：运行时默认 `lqr_pitch_sign=+1`，但当前 `LQR -> Tp -> VMC` 的组合符号链下，这会把第二输入通道按错误方向送入角向力矩执行。结果是 `phi/theta` 偏差经过 `Tp` 后，在画面上表现成后倒时前摆。

修复：把默认 `lqr_pitch_sign` 改为 `-1`，只翻转 `Tp` 的执行方向，不改 `theta_world` 定义、不改 `F_l` 正方向、不改状态维度。

下次避免：出现“某个状态偏差一出来，执行量明显朝视觉上错误方向动作”的情况时，先查最后一层执行符号链，不要先大改状态定义或再堆新增益。

## 2026-07-11：短 rollout 数据通过被误写成平衡成功

现象：曾把 `--virtual-rod-steps 200` 的 `behavior_qualified=True`、无饱和、腿长误差小、分支 violation 为 0 写成无扰动平衡恢复；实际可视化和长时间运行中，机器人仍会腿向反方向摆动、加速翻车。

根因：把程序化数据门禁当成了物理观察结论。短时 smoke 只能证明程序没立刻崩溃，不能覆盖数秒后才出现的失稳。

修复：撤销该结论。后续涉及平衡、摔倒、扰动恢复、前进运动时，必须给出可视化命令，由用户观察确认；数据指标只能辅助定位。

下次避免：不得用 `behavior_qualified`、短 rollout、腿长误差、分支 violation、饱和比例、接触力等指标单独宣称“已经平衡”。

## 2026-07-11：把实现失败解释为文章控制框架缺状态/缺控制量

现象：曾把当前控制器长时失稳解释成六状态模型缺少腿长/接触内部状态，进而把增广 `L/dL/delta_F_l` 当作可能主路径。

根因：没有严格区分“文章控制框架是否可行”和“当前实现是否正确”。参考文章的 LQR + VMC 轮腿控制在现实物理系统中已成功实现；本项目复现失败时，应优先检查实现中的符号、尺度、工作点、线性化和 VMC 映射。

修复：当前文档改为：文章式 `X=[theta,dtheta,x,dx,phi,dphi]^T`、`U=[T,Tp]^T` 是目标框架；不再用“缺状态/缺控制量”否定文章控制策略。

下次避免：文章已有解析解或解析公式时，必须先按解析式实现；只允许在解析解附近做局部工作点搜索，不允许用大范围数值优化或数据观察替代解析推导。
...
## 增广 LQR 不得继续冒充有效无扰动平衡控制器

把 `L/dL` 加入状态后若仍只保留 `[T,Tp]`，局部可控矩阵只有 `6/8` 阶；再加入 `delta_F_l` 虽然可以补齐输入维度，但会把 LQR 平衡层和 VMC 腿长支撑层混在一起。该路径没有恢复长时可视化平衡行为，因此当前已退出主控制链。

当前有效主路径恢复为 `X=[theta,dtheta,x,dx,phi,dphi]^T`、`U=[T,Tp]^T`；腿长只由 `F_l=F_l0+k_l(L0-L)-d_l dL` 支撑。需要抗扰时，先在这个已验证无扰动主路径上做外力脉冲测试，不再继续叠加未验证的增广 K。
