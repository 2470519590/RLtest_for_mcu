...
- Added the first task-aware residual RL reward in `server_training/reward.py`. The reward uses a compact LQR-like quadratic structure with task/phase weighting: posture and speed costs, normalized residual action and action-delta costs, medium saturation cost, touchdown impact penalty from wheel normal force and downward velocity, broad airborne leg-length range cost, jump liftoff bonus, early recovery bonus, and obvious-fall termination penalty. It does not reward in-place jump height and does not prescribe an exact leg compression trajectory after contact; impact reduction is the objective.
- The residual Env now returns `reward_terms` in `info`, and `run_train_residual_ppo.py` prints rollout-mean reward terms by default through a lightweight callback. This provides server-side feedback without requiring TensorBoard or viewer.
- Training Env jump tasks now enable an Env-only takeoff-control path: in-place jump enters the takeoff window shortly after the command, and forward jump enters once the speed profile reaches cruise. The old theta-ready and crouch-ready gates remain in ordinary smoke diagnostics, but the RL Env no longer waits for them. Leg-length residual action limits were widened to `0.08 m` so the policy can explore useful position-mode takeoff and airborne leg references within the existing safety clamp.
- Verification: `py_compile` passed for `server_training/reward.py`, the Env/training/smoke scripts, and `virtual_rod.py`. A 5-step `flight_ramp_medium` Env smoke ran and printed the widened action limits. `--compare-zero-residual` over a full 10 s `flight_ramp_medium` task still produced zero printed metric differences between `lqr` and `lqr_residual` with zero action. A short PPO smoke with `total_timesteps=40` printed reward term means and saved `runs/residual_ppo/reward_smoke/models/final_model.zip`.
- Added `run_residual_policy_eval.py` and `server_training/evaluate_policy.py` so a saved Stable-Baselines3 PPO `.zip` can be evaluated either headless or through the MuJoCo viewer. The evaluator keeps the residual Env task horizon intact and prints reward terms during the rollout.
- Updated the PPO training entry for server parallel use: `--subproc-start-method` is now explicit for `SubprocVecEnv`, startup prints the episode horizon and Env steps per episode, and reward-term logging now includes both global means and per-task buckets such as `reward_terms_by_task[...][flight_ramp_medium]`.
- Read the current long local training log from `C:\Users\24705\Desktop\log.txt`. The sampled tail showed `ep_len_mean` around `83..84` while a full `10 s / 0.02 s` task should be `500` Env steps, so that running process is mostly early-terminating before complete landing behavior can be trusted. The new per-task reward buckets only apply after restarting with the updated training script.
- Verification: `py_compile` passed for `server_training/evaluate_policy.py`, `run_residual_policy_eval.py`, `server_training/train_residual_ppo.py`, and `server_training/residual_env.py`. `run_residual_policy_eval.py --help` and `run_train_residual_ppo.py --help` both listed the expected new options. A short headless policy eval using the existing `runs/residual_ppo/reward_smoke/models/final_model.zip` loaded the model, stepped the Env, printed reward terms, and ended with `result: PASS residual policy eval`.
- Corrected the residual Env frequency semantics: low frequency belongs only to residual policy inference. MuJoCo physics, LQR, PID, VMC, limiters and safety logic must continue updating every 1 ms. `--step-seconds 0.02` means the residual action is refreshed every 20 ms and held; `control_decimation_steps=1` keeps the low-level controller at physics rate. The Env default was changed accordingly so forgetting the option no longer silently decimates the low-level controller.
- Added `run_export_residual_policy_onnx.py` and `server_training/export_policy_onnx.py` for deterministic actor export. Export output is the normalized 5-D residual action clipped to `[-1, 1]`; deployment should multiply by the residual action limits after inference.
- Verification: exported `runs/residual_ppo/final_model.zip` to `runs/residual_ppo/final_model.onnx` and checked ONNXRuntime against the PyTorch actor on zero observation. The maximum absolute error was about `1.49e-08`.

## STM32F407 CubeMX RL + LQR firmware skeleton

- Configured the generated CubeMX project at `runs/f407/RL_LQR` for the current landing residual policy. USART1 is currently set to `921600 8N1` on PA9/PA10 because this rate has been observed to work on the current hardware link; SYSCLK remains `168 MHz`, hard-float GCC flags are already enabled, and heap/stack reserves were raised to `0x2000`.
- Added an MCU-side control loop skeleton under `Core/Inc` and `Core/Src`: TIM2 update interrupt runs `AppControl_Tick1kHz` at 1 kHz with NVIC priority `1`; USART1 RX interrupt is priority `5`; SysTick remains the HAL time base. LQR, leg-length PID, VMC mapping, residual scaling, clamps and safety-limit placeholders run at the 1 kHz tick; the X-CUBE-AI residual network runs at `50 Hz` by default and holds its output between inference ticks.
- Added DWT cycle-counter timing around `ai_landing_run`, with `rl_last_us`, `rl_max_us`, `rl_frequency_hz`, `control_overruns`, and `ai_errors` stored in `AppControlStats`. USART telemetry reports `rl_last_us` and control outputs every `20 ms`.
- Current serial bridge accepts text `STATE`/`OBS` for fallback diagnostics and binary `STATEB` for normal serial-in-the-loop operation. `STATEB` sends drive-joint angles/rates so the MCU computes five-bar state and Jacobian locally before running LQR/PID/VMC/RL.
- Verification: CMake Debug and Release builds completed with STM32CubeCLT arm-none-eabi GCC 14.3.1. Release memory usage was about `39.8 KB FLASH` and `20.2 KB RAM`.
## STM32F407 MCU-side control algorithms

- Replaced the early firmware placeholders with MCU-side control calculations. The F407 project now computes analytic five-bar kinematics and Jacobian from left/right front/rear drive joint angles and rates, then uses the averaged virtual-leg state for LQR `theta/dtheta`.
- Ported the full `0.16..0.30 m` length-scheduled LQR table from `config/length_schedule.yaml` into C with linear interpolation for `X0`, `K`, and `F_l0`. Airborne nominal LQR keeps `T=0` and only the `Tp(theta,dtheta)` row, with residual injection still applied after the nominal output.
- The 1 kHz control tick now runs five-bar kinematics, LQR, yaw differential wheel torque, leg synchronization, roll-force compensation, leg-length PID, VMC mapping through `J^T [F_l,Tp]`, torque clamps and actuator gear conversion. The residual NN remains low-frequency at `50 Hz` and holds its last output between inference ticks.
- Added the `STATE` serial protocol for simulation-to-MCU validation. Unlike the legacy `OBS` debug path, `STATE` provides joint angles/rates so the MCU can compute virtual-leg state and Jacobian locally.
- Telemetry now reports residual inference latency (`rl_last_us`, `rl_max_us`), residual frequency, final torque summaries, actuator control values, overrun count and AI errors.
- Verification: Debug and Release CMake builds completed for `runs/f407/RL_LQR`. Release memory usage after the full MCU-side algorithm pass was about `49.2 KB FLASH` and `20.5 KB RAM`.

## STM32F407 serial-in-the-loop simulation bridge

- Clarified that the current F407 deployment target is serial-in-the-loop MuJoCo simulation, not real motor driving. MuJoCo sends simulated state through `STATE`, the F407 computes all control algorithms locally, and MuJoCo applies the returned `OUT` actuator controls.
- Added `run_mcu_serial_bridge.py` as the minimal PC-side bridge. It initializes a flat standing MuJoCo pose, sends body state, wheel odometry, task/speed conditioning, contact-derived airborne flag, and four drive-joint angles/rates to the MCU, then applies the latest six returned actuator controls to MuJoCo `data.ctrl`.
- Verification: `py_compile` passed for `run_mcu_serial_bridge.py`. Hardware-in-the-loop behavior still requires flashing the F407 and running the bridge with the actual serial port.
- Fixed the first bridge run issues after serial communication succeeded at `921600`: the PC bridge now prewarms the MuJoCo five-bar closed-loop constraints with position control before starting the serial loop, so the wheel/leg mechanism does not start from a broken passive-joint configuration. It also parses `OUT` fields `11..16` as the six actuator controls; field `10` is `airborne` and must not be used as a control value. Console output now includes `T/Tp/Fl`, overrun count, and AI error count for diagnosing whether zero actuator output comes from the MCU control chain or from bridge parsing.
- Verification: `py_compile` passed for `run_mcu_serial_bridge.py`. A local no-serial prewarm check produced finite qpos, `base_z=0.375`, and left/right lower-loop errors of about `8.8e-6 m`.
- Added the first binary state protocol for the F407 serial-in-the-loop path. PC frames use `0xA5 0x5A + seq_u16 + 29 float32 + xor_checksum`; the F407 accepts this as `STATEB` while still keeping the text `STATE` fallback. The PC bridge now defaults to `--protocol binary --state-hz 300`, because text `STATE` at high rate caused `seq` to stall and the current USB-UART link did not respond at 2 Mbps. The F407 telemetry uses integer-scaled `OUTI` fields so it does not depend on newlib-nano float printf support.
- Extended telemetry diagnostics with binary frame count, binary error count, text frame count, no-input ticks, and five-bar error count. This separates serial packet loss from controller computation failures.
- Control logic note: the temporary `locked` initial pose is geometrically valid but has virtual-leg length near `0.40 m`, outside the current `0.16..0.30 m` MCU length schedule and clamp range. It is useful for serial/control-chain diagnostics, but not a valid final balance test for the current `0.24 m` work point.
- Corrected the serial-in-the-loop default initial pose to the existing 0.24 m equilibrium initializer instead of the 0.40 m locked diagnostic pose. Verification showed `base_z=0.374960`, left leg `L=0.239961`, normal branch violation `0`, and lower-loop error about `3.2e-7`.
- Added UART error recovery for the byte-interrupt receiver so overrun/framing/noise events reset the binary parser and restart `HAL_UART_Receive_IT`.
- Split high-rate control from low-rate text telemetry. The MCU now emits compact binary `CTRLB` frames (`0xC3 0x3C + tick_u32 + seq_u16 + 6*int16 ctrl + xor_checksum`) from the background loop when a new 1 kHz control tick is available. The PC bridge parses `CTRLB` and applies it to MuJoCo; `OUTI` remains diagnostic only. Verification: `py_compile` passed for `run_mcu_serial_bridge.py`, and F407 Release CMake build passed with about `50.6 KB FLASH` and `20.7 KB RAM`.
- A 2 Mbps USART attempt produced no `READY`/`OUTI`/`CTRLB` reception on the observed COM link, so the default was returned to `921600`. Faster links can still be revisited later with a confirmed USB-UART adapter and/or a more compact quantized `STATEB` packet.
- After a run showed `STATEB` freezing at one accepted input frame while old control continued to act, the bridge was changed to conservative synchronized pacing. The default `STATEB` rate is now `100 Hz`, paced by received `CTRLB` frames instead of blind periodic sending. The MCU still runs LQR/PID/VMC at `1 kHz`, but the current serial-in-the-loop sample exchange is intentionally throttled so the F407 byte-interrupt receiver is not overrun by simultaneous large input packets and output frames. `CTRLB` is currently emitted at `200 Hz`; this is a bridge/transport limit, not the MCU control tick rate. Verification: `py_compile` passed for `run_mcu_serial_bridge.py`, and F407 Release CMake build passed.
- Replaced byte-interrupt UART RX with USART1 DMA circular RX on DMA2 Stream2 and moved packet parsing to `AppSerial_Process()`. USART1 TX now uses DMA2 Stream7 and non-blocking transmit; if a previous TX is active, new `CTRLB`/`OUTI` frames are skipped rather than blocking RX. Added `STATEQ`, an int16 quantized state packet (`0xA6 0x6A + seq_u16 + 29*int16 + xor_checksum`) as the default bridge protocol while keeping float32 `STATEB` as fallback. `STATEQ` is 63 bytes instead of the 121-byte float32 `STATEB`; angles use about `1e-4 rad` resolution, most rates and odometry use about `1e-3` resolution, and one-hot/boolean fields use integer values. `OUTI` telemetry was reduced from 50 Hz to 5 Hz. Verification: `py_compile` passed for `run_mcu_serial_bridge.py`, and F407 Release CMake build passed with about `53.0 KB FLASH` and `21.6 KB RAM`.
- For MCU serial-in-the-loop bring-up, residual inference is now disabled by default with `APP_RL_ENABLED=0` so the baseline LQR/PID/VMC path can be debugged without policy residuals. The MCU LQR also neutralizes the absolute `x` position channel to the scheduled workpoint, matching the project rule that `x` is not a global position task; the `dx` velocity channel remains active. Verification: F407 Release CMake build passed with residual disabled, about `40.5 KB FLASH` and `19.4 KB RAM`.
- Corrected the MCU LQR virtual-leg angle input from body-frame five-bar shape angle to the Python-equivalent world/heading-frame angle: `theta_lqr = theta_body - pitch`, `dtheta_lqr = dtheta_body - pitch_rate`. The VMC mapping still uses the body-frame shape Jacobian for joint torques. The PC bridge now also arms airborne detection only after observing wheel contact, matching the documented reset guard and preventing initial zero contact force from forcing the LQR airborne gate. Verification: `py_compile` passed for the bridge and F407 Release CMake build passed.
- Added a serial `RESET` command for episode-level MCU session reset. The PC bridge sends it before each run; the MCU clears latest input, serial counters, control stats, output controls, residual buffers, and VMC PID memory, then waits for new state frames. This prevents stale pre-run MCU state from contaminating a new MuJoCo episode. Verification: `py_compile` passed for the bridge and F407 Release CMake build passed.
- Corrected MCU LQR output signs and limits to match `config/smoke.yaml`: `APP_LQR_WHEEL_SIGN=+1`, `APP_LQR_PITCH_SIGN=-1`, and `APP_LQR_TP_LIMIT=2.0`. The previous MCU path omitted the runtime pitch sign and used an 8 N*m pitch limit, which made the pitch channel drive in the wrong direction and saturate far beyond the current YAML controller. Also aligned `APP_LEG_SYNC_KD=20.0` and the leg-length integral limit with YAML. Verification: F407 Release CMake build passed.
- After the MCU baseline could balance with residual disabled, the serial bridge flight-ramp entry was aligned with the existing smoke flight test. `--task flight_ramp` now injects the same full-width ramp scene used by `run_smoke.py --flight-test`, and the state `dx` channel sent to the MCU is wheel odometry speed minus the trapezoid speed reference for the selected ramp speed. This keeps the MCU LQR interface as a local speed-error channel while allowing `--speed high` to reproduce the high-speed ramp approach. Residual inference remains disabled in firmware for this baseline check. Verification: `py_compile` passed for `run_mcu_serial_bridge.py`, and the injected flight-ramp MuJoCo model loaded with `0.001 s` timestep.
- Corrected the serial-in-the-loop authority boundary: flight-ramp state machines and control references must run on the MCU, not in the PC bridge. The bridge no longer computes airborne, speed-reference suppression, or landing task phases. Instead, `STATEB/STATEQ/STATE` now carry 31 raw/command fields: the previous airborne slot is `sim_time_s`, and the final two fields are left/right wheel normal forces from MuJoCo. The MCU parses these sensor fields, computes the flight-ramp trapezoid speed reference internally, performs contact arming and `50 ms` airborne confirmation internally, applies the paper-style airborne LQR gate internally, and suppresses the speed profile after the first confirmed airborne event so recontact commands a stop. Console output now prints `mcu_air` from MCU telemetry and `Fn_sensor` from raw bridge observation; the bridge-side values are diagnostic only and do not affect control. Verification: `py_compile` passed for `run_mcu_serial_bridge.py`; F407 Release build passed after the protocol/control move with about `41.3 KB FLASH` and `19.4 KB RAM`.
- Enabled MCU-side residual inference for the serial-in-the-loop test by setting `APP_RL_ENABLED=1`. The 1 kHz LQR/PID/VMC loop is unchanged; the X-CUBE-AI residual policy runs at `50 Hz` through `RlPolicy_Run()` and the most recent residual output is held between inference ticks. Verification: F407 Release build passed with residual enabled, using about `53.8 KB FLASH` and `21.6 KB RAM`.
- Increased MCU-side residual inference from `50 Hz` to `1 kHz` after the measured NN inference latency was about `0.25 ms` and the user confirmed behavior looked reasonable. The residual policy now runs every control tick instead of holding between ticks; LQR/PID/VMC still run in the same 1 kHz loop. Runtime validation should watch `rl_us`, `rl_max_us`, and `overrun` to ensure the F407 timing margin remains acceptable. Verification: F407 Release build passed with about `53.8 KB FLASH` and `21.6 KB RAM`.
